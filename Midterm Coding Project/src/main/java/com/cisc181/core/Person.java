...
public abstract class Person implements java.io.Serializable {
	...
	public void DOBException(Person person) throws PersonException {
		Calendar currentDate = Calendar.getInstance();
		currentDate.add(Calendar.YEAR, -100);
		if (person.DOB.before(currentDate.getTime())) {
			throw new PersonException(person);
		}
	public void PhoneNumberException(Person person) throws PersonException{
		
		Pattern pattern = Pattern.compile("^\\(?([0-9]{3})\\)?[-.\\s]?([0-9]{3})[-.\\s]?([0-9]{4})$");
		Matcher matcher = pattern.matcher(person.getphone_number);
